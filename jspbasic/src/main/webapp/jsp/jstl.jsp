...
<%-- 변수 출력 (변수의 값을 출력할땐 표현식 사용)--%>
<c:out value="${v}" /><br>

<%-- 변수 제거 --%>
<c:remove var="v" scope="page"/>

<%-- 변수 출력 --%>
<c:out value="${v}" /><br>

<%-- 조건문 --%>
<c:set var="v1" value="4"/>
<c:set var="v2" value="6"/>
<c:if test="${v1>v2}">
	<c:out value="v1이 v2보다 큽니다!"/><br>
</c:if>
<c:if test="${!v1>v2}">
	<c:out value="v1이 v2보다 크지 않습니다!"/><br>
</c:if>
${v1>v2 ? "v1이 v2보다 큽니다" : "v1이 v2보다 크지 않습니다"}<br>
...
<%--
fmt:bundle : properties 파일 사용시, 태그 안에서만
basename : package명 + properties 파일명
prefix : 키 앞에 붙일 문자열

fmt:setBundle : 설정 후부터 properties 사용 가능
--%>

<%-- properties 키값 앞에 hong_안붙이면 prefix 없어도됨 --%>
...
<c:out value='${fn:replace(str, "Hello", "Bye")}'/><br>
...
<%--
fn:escapeXml : 태그를 해석하지 않고 태그 자체로 표시
--%>
<c:set var="html" value="<hr><p>하이롱롱</p></hr>"/>
